<div align="center">

# Hi, I'm Pralay Bajkhan 👋

[![Typing SVG](https://readme-typing-svg.demolab.com/?font=Fira+Code\&weight=500\&size=22\&pause=1000\&color=2E9EF7\&center=true\&vCenter=true\&width=600\&height=50\&lines=AI+%26+Machine+Learning+Graduate;Data+Analytics+Enthusiast;Python+%7C+SQL+%7C+Machine+Learning;Building+Data-Driven+Solutions)](https://git.io/typing-svg)

### AI & Machine Learning Graduate | Data Analytics | Python | SQL

🎓 **B.Tech CSE (AI & ML)** • Brainware University • **CGPA: 8.87/10**
📍 West Bengal, India • 💼 Open to **Entry-Level Opportunities & Collaborations**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge\&logo=linkedin\&logoColor=white)](https://www.linkedin.com/in/pralay-bajkhan-4892a4200)
[![Portfolio](https://img.shields.io/badge/Portfolio-Visit-2E9EF7?style=for-the-badge\&logo=google-chrome\&logoColor=white)](https://spiffy-youtiao-009ed2.netlify.app/)
[![Email](https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge\&logo=gmail\&logoColor=white)](mailto:pralaybajkhan116@gmail.com)

</div>

---

## 🚀 About Me

I'm a **B.Tech graduate in Computer Science & Engineering (AI & ML)** with hands-on experience in **Machine Learning, Data Analytics, Python, SQL, and Business Intelligence**.

I enjoy building practical, data-driven applications that solve real-world problems — from **demand forecasting and fraud detection** to **business analytics and NLP applications**.

* 🤖 Interested in **Artificial Intelligence & Machine Learning**
* 📊 Passionate about **Data Analytics & Business Intelligence**
* 🐍 Working with **Python, Pandas, NumPy & Scikit-Learn**
* 🗄️ Building analytical solutions using **SQL**
* 📈 Creating dashboards and visualizations with **Power BI**
* 🧠 Exploring **NLP, Generative AI & Predictive Modeling**
* ⚡ Building and deploying practical AI-powered applications
* 🔍 Looking for opportunities to apply my skills to real-world problems

---

## 🛠️ Tech Stack

### 💻 Programming & Data

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" alt="Python" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/c/c-original.svg" alt="C" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pandas/pandas-original.svg" alt="Pandas" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/numpy/numpy-original.svg" alt="NumPy" width="45" height="45"/>
</p>

**Python • C • SQL • Pandas • NumPy**

### 🤖 AI & Machine Learning

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/scikitlearn/scikitlearn-original.svg" alt="Scikit-Learn" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tensorflow/tensorflow-original.svg" alt="TensorFlow" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pytorch/pytorch-original.svg" alt="PyTorch" width="45" height="45"/>
</p>

**Machine Learning • Scikit-Learn • TensorFlow • PyTorch • NLP • Generative AI**

### 📊 Analytics & Visualization

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/matplotlib/matplotlib-original.svg" alt="Matplotlib" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/seaborn/seaborn-original.svg" alt="Seaborn" width="45" height="45"/>
</p>

**Power BI • Excel • Matplotlib • Seaborn • Data Analysis • Data Visualization**

### 🌐 Development & Tools

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/fastapi/fastapi-original.svg" alt="FastAPI" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" alt="React" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vitejs/vitejs-original.svg" alt="Vite" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" alt="Git" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" alt="GitHub" width="45" height="45"/>
</p>

**FastAPI • React • Vite • Git • GitHub • VS Code • Jupyter • Google Colab**

---

# 🚀 Featured Projects

### 🔮 FORESIGHT — Demand & Inventory Intelligence

AI-powered demand and inventory intelligence application designed to **forecast Store-SKU demand and identify inventory risks**.

**Highlights:**

* 📊 Store-SKU demand forecasting
* 🧠 Feature engineering and predictive modeling
* 📦 Inventory risk and replenishment analysis
* ⚡ FastAPI backend with interactive React dashboard
* ☁️ Deployed application

**Tech:** Python • Pandas • Scikit-Learn • Machine Learning • Forecasting • FastAPI • React

🔗 [**Live Demo**](https://foresight-demand-frontend.onrender.com/) • 💻 [**Source Code**](https://github.com/Pralay567/FORESIGHT)

---

### 💳 Financial Fraud Detection

AI-powered fraud detection application that analyzes transaction attributes to estimate **fraud probability and risk levels**.

**Highlights:**

* 🧠 Tuned HistGradientBoosting model
* 📈 Fraud probability and risk scoring
* 🚨 Risk classification
* ⚡ FastAPI REST API
* 💻 Interactive React dashboard
* ☁️ Deployed for real-time prediction

**Tech:** Python • Pandas • Scikit-Learn • Machine Learning • FastAPI • React

🔗 [**Live Demo**](https://financial-fraud-detection-frontend.onrender.com/) • 💻 [**Source Code**](https://github.com/Pralay567/financial-fraud-detection)

---

### 📊 Business Analytics Dashboard

Interactive business intelligence application for analyzing **sales performance, customer behavior, and key business KPIs**.

**Highlights:**

* 📈 Sales and KPI analysis
* 🗄️ SQL-based data analysis
* 🧹 Data cleaning and transformation
* 📊 Interactive visualizations
* 💡 Business insights and reporting

**Tech:** Python • Pandas • SQL • Power BI • Data Analytics • Streamlit

🔗 [**Live Demo**](https://global-business-analytics-dashboard-o2xvjzfd4sjrmhfrcw6qss.streamlit.app/) • 💻 [**Source Code**](https://github.com/Pralay567/Global-Business-Analytics-Dashboard)

---

### 📰 Fake News Detection System

NLP-based machine learning application that classifies news articles as **real or fake**.

**Tech:** Python • NLP • TF-IDF • Scikit-Learn • Machine Learning • Streamlit

🔗 [**Live Demo**](https://fake-news-detection-ml-tmfdyzjga4fhsgcx9pwes6.streamlit.app/) • 💻 [**Source Code**](https://github.com/Pralay567/fake-news-detection-ml)

---

### 🤖 AI Career Copilot

AI-powered career guidance application that analyzes **skills, resumes, career goals, and student profiles** to provide personalized career insights.

**Features:** Skill-Gap Analysis • Career Guidance • Interview Preparation • Learning Recommendations

**Tech:** Python • Machine Learning • NLP • Generative AI • Streamlit

🔗 [**Live Demo**](https://ai-career-copilot-pnvc7nrxhdlzh7o9vakmpf.streamlit.app/) • 💻 [**Source Code**](https://github.com/Pralay567/AI-Career-Copilot)

---

### 🗳️ Election Guide Assistant

AI-powered application providing **constituency information, election guidance, and location-based services**.

**Tech:** Python • Gemini AI • Streamlit • Folium • Geopy

🔗 [**Live Demo**](https://election-assistant-fdgxetefqsxjy4dqu2j8ra.streamlit.app/) • 💻 [**Source Code**](https://github.com/Pralay567/election-assistant)

---

## 💼 Experience

### 🔹 Machine Learning Intern — Saiket Systems

**July 2026 – August 2026 | Remote**

* Performed data cleaning, preprocessing, EDA, and statistical analysis using Python, Pandas, and NumPy.
* Worked on feature preparation, machine learning techniques, and data visualization.
* Analyzed datasets to identify patterns and support data-driven solutions.

### 🔹 Data Analytics Intern — Zidio Development

**2026 | Remote**

* Performed data cleaning, preprocessing, EDA, and data manipulation using Python, Pandas, and SQL.
* Created visualizations and analytical outputs to identify trends and communicate insights.
* Worked on practical data analytics and machine learning projects.

---

## 🎓 Education

### B.Tech in Computer Science & Engineering (AI & ML)

**Brainware University | 2022 – 2026**

🎓 **CGPA: 8.87/10**

**Focus Areas:**
Artificial Intelligence • Machine Learning • Data Science • Data Analytics • NLP • Generative AI • Python • SQL

---

## 📜 Certifications

* 🏅 **Oracle Cloud Infrastructure — AI Foundations Associate**
* ☁️ **Google Cloud Generative AI Academy APAC — Hackathon**
* 🤖 **AI/ML Using Python — Simplilearn**
* 🧠 **Introduction to Artificial Intelligence — Simplilearn**
* 📊 **Data Visualization using Python & Seaborn — Microsoft Learn / DevTown**
* 🤖 **AI/ML/DL — EdGate / Intel Partner**
* 📈 **TATA — GenAI Powered Data Analytics — Forage**
* 📊 **Deloitte — Data Analytics — Forage**
* ☁️ **Deloitte — Solution Architecture — Forage**

---

## 📊 GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=Pralay567&show_icons=true&theme=tokyonight&hide_border=true" alt="Pralay's GitHub Stats" />

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Pralay567&layout=compact&theme=tokyonight&hide_border=true" alt="Pralay's Top Languages" />

</div>

---

## 📌 GitHub Activity

<div align="center">

![GitHub followers](https://img.shields.io/github/followers/Pralay567?style=for-the-badge\&logo=github\&label=Followers)

![GitHub stars](https://img.shields.io/github/stars/Pralay567?style=for-the-badge\&logo=github\&label=Total%20Stars)

![Profile Views](https://komarev.com/ghpvc/?username=Pralay567\&label=Profile%20Views\&style=for-the-badge)

</div>

---

## 🤝 Let's Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge\&logo=linkedin\&logoColor=white)](https://www.linkedin.com/in/pralay-bajkhan-4892a4200)

[![Portfolio](https://img.shields.io/badge/Portfolio-Visit-2E9EF7?style=for-the-badge\&logo=google-chrome\&logoColor=white)](https://spiffy-youtiao-009ed2.netlify.app/)

[![Email](https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge\&logo=gmail\&logoColor=white)](mailto:pralaybajkhan116@gmail.com)

[![GitHub](https://img.shields.io/badge/GitHub-Pralay567-181717?style=for-the-badge\&logo=github\&logoColor=white)](https://github.com/Pralay567)

</div>

---

<div align="center">

### 💡 Building with data. Learning with AI. Creating practical solutions.

</div>

---

## 🤝 Let's Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge\&logo=linkedin\&logoColor=white)](https://www.linkedin.com/in/pralay-bajkhan-4892a4200)

[![Portfolio](https://img.shields.io/badge/Portfolio-Visit-2E9EF7?style=for-the-badge\&logo=google-chrome\&logoColor=white)](https://spiffy-youtiao-009ed2.netlify.app/)

[![Email](https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge\&logo=gmail\&logoColor=white)](mailto:pralaybajkhan116@gmail.com)

[![GitHub](https://img.shields.io/badge/GitHub-Pralay567-181717?style=for-the-badge\&logo=github\&logoColor=white)](https://github.com/Pralay567)

</div>

---

<div align="center">

### 💡 Building with data. Learning with AI. Creating practical solutions.

![Profile Views](https://komarev.com/ghpvc/?username=Pralay567\&label=Profile%20Views\&color=0e75b6\&style=flat)

[![GitHub followers](https://img.shields.io/github/followers/Pralay567?label=Follow\&style=social)](https://github.com/Pralay567)

</div>
